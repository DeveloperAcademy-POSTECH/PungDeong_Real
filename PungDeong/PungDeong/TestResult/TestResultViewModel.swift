...
class TestResultViewModel: NSObject {
    ...
    func resultType(_ type: Int) -> resultData {
        ...
        switch type {
            case 1:
                result = resultData(type: "1", name: "프로 팩트체커 교직원 우파루파", hastag: ["슬기로운_팩트체크생활", "의심쟁이"], description: "당신은 팩트체크가 생활화된 우파루파! 사소한 정보 하나라도 그냥 넘겨짚지 않고 팩트체크를 하는 모습이 마치 성실하고 유능한 변호사 같아요. 불확실한 사실의 폭풍으로부터 마을을 지켜주는 한 줌의 빛이 되어주는 것은 어떤가요?", image: "character_1")
            case 2:
                result = resultData(type: "2", name: "박학다식 대학원생 해달", hastag: ["TMI", "박사까지_가즈아~!"], description: "당신은 전문 지식을 중시하는 해달이에요! 사실 여부가 확실하지 않은 정보를 접하면 그냥 받아들이기 보다는 전문가의 조언을 구하는 당신의 모습이 마치 척척석사 같군요. 주변 이웃들이 어려움을 겪을 때 당신의 박학다식함으로 도움을 주는 건 어떤가요?", image: "character_2")
            case 3:
            ...
            case 4:
                result = resultData(type: "4", name: "논리적인 수험생 돌고래", hastag: ["논리만렙", "범생이"], description: "논리가 중요한 당신은 논술 만점에 빛나는 돌고래! 근거가 빈약한 주장을 꺼려하는 한편, 근거가 탄탄하다면 반대되는 주장도 받아들일 준비가 되어있어요. 이런 당신의 모습은 마치 논술 만점을 받은 우등생 같네요! 당신의 뛰어난 논리력으로 문제에 처한 이웃들을 도와주는 건 어떨까요?", image: "character_4")
            case 5:
